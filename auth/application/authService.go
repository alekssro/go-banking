...
func (s DefaultAuthService) Refresh(request dto.RefreshTokenRequest) (*dto.LoginResponse, *errs.AppError) {
	if vErr := request.IsAccessTokenValid(); vErr != nil {
		if vErr.Errors == jwt.ValidationErrorExpired {
			...
			if accessToken, appErr = entity.NewAccessTokenFromRefreshToken(request.RefreshToken); appErr != nil {
				return nil, appErr
			}
...
func (s DefaultAuthService) Login(req dto.LoginRequest) (*dto.LoginResponse, *errs.AppError) {
	...
	authToken := entity.NewAuthToken(claims)
	...
	if jwtToken, err := jwtTokenFromString(urlParams["token"]); err != nil {
		...
		*/
		if jwtToken.Valid {
			...
			claims := jwtToken.Claims.(*entity.AccessTokenClaims)
...
func jwtTokenFromString(tokenString string) (*jwt.Token, error) {
	token, err := jwt.ParseWithClaims(tokenString, &entity.AccessTokenClaims{}, func(token *jwt.Token) (interface{}, error) {
		return []byte(entity.HMAC_SAMPLE_SECRET), nil
	})
	if err != nil {
...
func NewLoginService(repo repository.AuthRepository, permissions entity.RolePermissions) DefaultAuthService {
